...
APlayer.prototype.init = function (auto_play, reload) {
                        ...
                        '<i class="demo-icon aplayer-icon-volume-down"></i>' +
                            '<div class="aplayer-volume-bar-wrap">' +
                                '<div class="aplayer-volume-bar">' +
                                    '<div class="aplayer-volume" style="width: 100%;"></div>' +
                            '</div>' +
                        '</div>' +
                    '</div>' +
                '</div>' +
    ...
    this.element.getElementsByClassName('aplayer-volume-bar-wrap')[0].addEventListener('click', function (event) {
        var e = event || window.event;
        var percentage = (e.clientX - getElementViewLeft(volumeBarWrap)) / barWidthV;
        percentage = percentage > 0 ? percentage : 0;
        percentage = percentage < 1 ? percentage : 1;
        _self.updateBar.call(_self, 'volume', percentage, 'width');
    ...
    volumeicon.addEventListener('click', function () {
        if (_self.audio.muted) {
            ...
            _self.updateBar.call(_self, 'volume', _self.audio.volume, 'width');
        }
        else {
            _self.audio.muted = true;
            volumeicon.className = 'demo-icon aplayer-icon-volume-off';
            _self.updateBar.call(_self, 'volume', 0, 'width');
        }
    });
...
APlayer.prototype.setMusic = function (index, reload) {
    ...
    if ((this.playIndex > -1 && !this.audios[indexMusic]) || this.playIndex === -1 || reload) {
        ...
        // control volume
        this.audio.volume = parseInt(this.element.getElementsByClassName('aplayer-volume')[0].style.width) / 100;
